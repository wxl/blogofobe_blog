...
def load_env():
    # TODO: Get rid of globals and move imports to top of file, if possible.
    global tools, post
    from . import tools
...
def setup_parser(parent_parser, parser_template):
    ...
    blog_templates_copy.add_argument(
        "DEST",
        help="Destiation to copy templates to")
...
def copy_templates(args):
    ...
    try:
        shutil.copytree(
            os.path.join(tools.get_src_dir(), "_templates","blog"), args.DEST)
    except OSError:
        print(
            "Destination ({0}) already exists.\n"
            "You must choose a path that does not yet exist."
            .format(args.DEST))
        return
